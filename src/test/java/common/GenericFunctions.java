...
public class GenericFunctions {
    ...
    static HashMap<String, String> requestUrls = new HashMap<>();
    ...
    public void createsRequest(String key, String endpoint) {
        if (!reqSpecMap.containsKey(key)) {
            RequestSpecification reqSpec = SerenityRest.given().spec(requestSpecifications(endpoint, key));
     ...
     */
    public RequestSpecification requestSpecifications(String endpoint, String key) {
        RequestSpecification req = SerenityRest.given();
        String basePath = EnvironmentSpecificConfiguration.from(variables).getProperty(endpoint);

        // Check if query parameters are present in the endpoint
        if (basePath.contains("?")) {
            // Add query parameters if present in the endpoint
            String[] paramArray = basePath.split("\\?")[1].split("&");
        ...
        int queryParamIndex = basePath.indexOf("?");
    ...
    public void sendRequest(String requestType) {
        ...
        try {
            Response response = reqSpecMap.get(latestRequestKey).log().all().when().request(requestType,requestUrls.get(latestRequestKey));
            logger.logInfo("Response Body: \n\n" + response.getBody().asPrettyString());
            latestResponseKey=latestRequestKey;
    ...
    public void sendRequest(String requestType, String requestKey) {
        ...
        requestType = requestType.toUpperCase();
        try {
            response = reqSpecMap.get(requestKey).log().all().when().request(requestType,requestUrls.get(requestKey));
        } catch (Exception e) {
            logger.logInfo("Unsupported method: " + requestType);
        }
    ...
    public void addBody(String methodBody) {
        reqSpecMap.get(latestRequestKey).log().all().when().body(jsonBody(methodBody, environment));
    }
    ...
    public void setContentType(String contentType) {
        reqSpecMap.get(latestRequestKey).contentType(contentType);
    }
    ...
    public void verifyStatusCode(int statusCode) {
        Assert.assertEquals("Status Code mismatch", statusCode, responseMap.get(latestResponseKey).getStatusCode());
    ...
    public void addHeaders(String headerName, String headerValue) {
        reqSpecMap.get(latestRequestKey).header(headerName, headerValue);
    }
    ...
    public void addHeaders(DataTable dt) {
        if (validateDataTableHeaders(dt, "Header Name", "Header Value")) {
            ...
            for (Map<String, String> header : headers) {
                reqSpecMap.get(latestRequestKey).header(header.get("Header Name"), header.get("Header Value"));
            }
        }
    }
    ...
    public void addHeadersViaMap(Map<String, String> headers) {
        reqSpecMap.get(latestRequestKey).headers(headers);
    }
    ...
    public void addQueryParams(DataTable dt) {
        if (validateDataTableHeaders(dt, "Parameter Key", "Parameter Value")) {
            ...
            for (Map<String, String> params : queryParams) {
                reqSpecMap.get(latestRequestKey).queryParam(params.get("Parameter Key"), params.get("Parameter Value"));
            }
        }
    }
    ...
    public void addQueryParamsViaMap(Map<String, String> queryParam) {
        reqSpecMap.get(latestRequestKey).queryParams(queryParam);
    }
    ...
    public void addFormParams(String parameterKey, String parameterValue) {
        reqSpecMap.get(latestRequestKey).formParam(parameterKey, parameterValue);
    }
    ...
    public void addFormParams(DataTable dt) {
        if (validateDataTableHeaders(dt, "Parameter Key", "Parameter Value")) {
            ...
            for (Map<String, String> params : formParams) {
                reqSpecMap.get(latestRequestKey).formParam(params.get("Parameter Key"), params.get("Parameter Value"));
            }
        }
    }
    ...
    public void addFormParamsViaMap(Map<String, String> formParams) {
        reqSpecMap.get(latestRequestKey).formParams(formParams);
    }
    ...
    public void addPathParams(String parameterKey, String parameterValue) {
        ...
        reqSpecMap.get(latestRequestKey).pathParam(parameterKey, parameterValue);
    }
    ...
    public void addPathParams(DataTable dt) {
        if (validateDataTableHeaders(dt, "Parameter Key", "Parameter Value")) {
            ...
            for (Map<String, String> params : pathParams) {
                reqSpecMap.get(latestRequestKey).pathParam(params.get("Parameter Key"), params.get("Parameter Value"));
            }
        }
    }
    ...
    public void addPathParamsViaMap(Map<String, String> pathParams) {
        reqSpecMap.get(latestRequestKey).pathParams(pathParams);
    }
    ...
    public void extractValue(String key) {
        extractedValue.put(key, responseMap.get(latestRequestKey).jsonPath().getString(key));
    }

    /**
     * Extract value of a key from the latest response
     *
     * @param keys The key whose value needs to be extracted
     */
    ...
    /**
     * Adds the extracted key value from another response body to the request body of the latest request
     *
    ...
    public void addExtractedValueToRequest(String keyToAdd, String newKeyPath, String methodBody) {
        reqSpecMap.get(latestRequestKey).log().all().when().body(jsonBody(methodBody, environment, newKeyPath, extractedValue.get(keyToAdd), true));
    }
            ...
            if (!responseMap.containsKey(responseKey)) {
                ...
                if (ObjectUtils.allNull(valueFoundAtPath)) {
                    if (size == 0) {
                        ...
                        logger.logError("No value found for provided key: \"" + pathProvided + "\"");
                        Assert.fail("No value found for key: \"" + pathProvided + "\"");
                    }
                } else if (valueFoundAtPath instanceof ArrayList) {
                    ArrayList arr = (ArrayList) valueFoundAtPath;
                    arrSize = arr.size();
    ...
    public void verifyResponseSchema(String schemaName, String responseKey) {
        if (!responseMap.containsKey(responseKey)) {
            ...
            try {
                ...
                if (e.getMessage().contains("JsonMappingException") || e.getMessage().contains("IOException") || e.getMessage().contains("FileNotFoundException") || e.getMessage().contains("JsonParseException")) {
                    logger.logError("Failed to validate Schema. Body may not match expectations. Exception: " + e.getMessage());
                    Assert.fail("Failed to validate Schema. Body may not match expectations. Exception: " + e.getMessage());
                } else {
                    logger.logError("Failed to validate Schema due to Exception: " + e);
    ...
    public void setUrlEncoding(boolean encodingStatus) {
        reqSpecMap.get(latestRequestKey).urlEncodingEnabled(encodingStatus);
    }
    ...
    public void setRelaxedHttpsValidation() {
        reqSpecMap.get(latestRequestKey).relaxedHTTPSValidation();
    }
    ...
    public void addMultipartFileToRequest(String controlName, String filename) {
        ...
        reqSpecMap.get(latestRequestKey).multiPart(controlName, file);
    }
    ...
    public void addMultipartFileToRequestWithMIMEType(String controlName, String filename, String mimeType) {
        ...
        File file = new File("src/test/resources/multipartfiles/" + filename);
        reqSpecMap.get(latestRequestKey).multiPart(controlName, file, mimeType);
    }
    ...
    public void addBasicAuth() {
        ...
        reqSpecMap.get(latestRequestKey).auth().basic(username, password);
    }
    ...
    public void addBearerAuth() {
        ...
        reqSpecMap.get(latestRequestKey).headers("Authorization", "Bearer " + token);
    }
    ...
    public void performComparison(String response1, String responseKey2) {
